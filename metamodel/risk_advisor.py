...
"tree_method":['hist'],
"grow_policy":['depthwise','lossguide','depthwise','lossguide','lossguide','depthwise','lossguide','depthwise','lossguide','lossguide'],
...
class riskadvisor_model:
    ...
    def create_sense_model(self,num_models=10,objective='multi:softmax'):
        xgb_models=[]
        for i in range(num_models):
            clf=xgb.XGBClassifier(device=self.device,n_estimators=parameters["n_estimators"][i],
                                learning_rate=parameters["lr"][i],max_depth=parameters["max_depth"][i],subsample=parameters["subsample"][i],
                                tree_method='hist',grow_policy=parameters["grow_policy"][i],min_child_weight=parameters["min_child_weight"][i],
    ...
    def gen_calib_data(self,train_x,train_y,train_loss,balance=False,test_size=0.1):

        train_y=train_y.reshape(-1,1)
        train_loss=train_loss.reshape(-1,1)
        train_labels=np.concatenate([train_y,train_loss],axis=1)
        # train_x, calib_x, train_labels, calib_labels = train_test_split(train_x, train_labels, test_size=test_size)
        train_x, calib_un_x, train_labels, calib_un_labels = train_test_split(train_x, train_labels, test_size=test_size)
    ...
    def inf(self,X,apply_temp=False):
