...
from ..utils.utils import DatasetType, train_test_val_split_on_column,train_test_val_split_on_file
...
class DataHandler:
    ...
    def split_data(
        ...
        self.index_splits = {
            split: df['id'].to_list() for split, df in df_splits.items()
        }
        return self

    def split_data_file(
        self,df_train,df_test,df_val
    ) -> "DataHandler":
        """Split the data into train, test and validation sets,
        split indices are stored in self.index_splits.
        ...
        df_splits = train_test_val_split_on_file(df_train,df_test,df_val)
        self.index_splits = {
            split: df['id'].to_list() for split, df in df_splits.items()
        }
        return self
    ...
    def _get_split_iterator_custom(self, split: DatasetType) -> BaseIterator:
        keys = self.index_splits[split]
        iterator = self.data_processor.make_iterator(
            keys=self.index_splits[split], data=self.full_df.loc[keys], split=split
        )
    ...
    def get_iterators_file(self,df_train,df_test,df_val) -> Dict[DatasetType, BaseIterator]:
        """Return a dictionary of iterators for the different splits of the data.
        ...
        if not self.index_splits:
            logger.warning("No data split yet. Splitting data now.")
            self.split_data_file(df_train,df_test,df_val)
        return {split: self._get_split_iterator_custom(split) for split in self.index_splits}
