...
from metamodel.tune_temp_scaler_query import search_best_temp_scaler
from metamodel.gen_train_data import build_metamodel_training_set, create_loss_labels, generate_image_embeddings_v2
from quantile_regressor.quantile_model_old import QuantileErrorEstimator
...
class MetamodelPipeline:
    def __init__(self, workspace_dir, task):
        ...
        self.meta_model = metamodel(device="cpu")
    ...
    def run(self, base_model, X_base, X_meta, y_true, force=False, use_embeddings=False, apply_temp_scaling=False):
        """
        ...
        if not self._exists("split_calib_bal_x.npy", "data") or force:
            ...
            splits = self.meta_model.gen_calib_data(
                ...
                train_y=meta_y, 
                train_loss=losses,
                balance=True # CRITICAL: Balance the calibration set
            )
        ...
        if not os.path.exists(q_model_path) or force:
            ...
            self.q_estimator.fit(
                ...
                apply_temp_scaling=apply_temp_scaling
            )
        ...
        self.q_estimator.calibrate_coverage(self.meta_model,
                train_x_scaled,train_y,
                apply_temp_scaling=apply_temp_scaling
            )
        
        # --- NEW ADDITION: ESTABLISH BASELINE STATS ---
        print(">>> [8/8] Estimating Baseline Uncertainty Drift...")
        self.q_estimator.estimate_uncertainty_drift(
            sense_model=self.meta_model, 
            X_ref=train_x_scaled, 
            X_chrono_train=train_x_scaled, 
            window=100, 
            tests=1000, 
            apply_temp_scaling=apply_temp_scaling
        )
