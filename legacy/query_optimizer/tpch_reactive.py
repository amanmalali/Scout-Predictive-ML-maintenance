...
def run_sim(data,alpha_model,sense_model,labeler,delta=1*60*60,reg=None,distro=None,avg_train_loss=-1,sc=None,split_iterators=None,kappa=6000):
    uncer_counter=0
    count=0
    predicted_loss=0
    predicted_lower=0
    ...
    for index,row in data[random_start:].iterrows():
        ...
        new_entry=np.hstack([arr,[alpha_model_pred.cpu().detach().numpy()[0][0],in_y,alpha_loss,loss_label,-1,-1,-1,-1,max_loss,total_arrivals,predicted_loss]])
        ...
        if len(storage_data)>0:
            if storage_data.iloc[[-1]]['timestamp'].values[0]>retrain_ts:
                new_close_ts=find_nearest(np.array(ts),retrain_ts)
                if (storage_data['loss_label'][new_close_ts:].sum()-avg_train_loss*len(storage_data['loss_label'][new_close_ts:]))>kappa:
                    ...
                    start=time.time()
                    new_retrain_df,new_retest_df,new_model_path,model=retrain_model(new_train_df,dir_name="./checkpoints_retrain/reactive_v1_tpch_kappa="+str(kappa)+"_"+str(int(delta))+"_"+str(sc))
    ...
    # print(split_iterators["train"].shape)
                    ...
                    # new_retrain_df.to_csv('./dummy_data/reactive_retrain_v0_fixed_'+str(delta)+'_'+str(sc)+'.csv')
                    ...
                    np.save('./data/retraining_ts_reactive_v1_tpch_kappa='+str(kappa)+'_'+str(delta)+'_'+str(sc)+'.npy',retraining_timestamps)
    ...
    storage_data.to_csv("./data/results_reactive_v1_tpch_kappa="+str(kappa)+"_"+str(delta)+"_"+str(sc)+".csv")
...
run_sim(future_data,alpha_model,sense_model,labeler,delta=delta*60*60,reg=reg,distro=full_dist,avg_train_loss=train_y.mean(),sc=sc,split_iterators=initial_iterator,kappa=kappa)
