...
class linear_model(nn.Module):
    ...
    def __init__(self, in_features: int, K: int = 5,
                 T_min: float = 0.7, T_max: float = 7.0):
        super().__init__()
    ...
    def forward(self, x):
        t_sq  = self.lin(x).pow(2)
        # temps = F.softplus(t_sq, beta=1)
...
def train_scaler(data, epochs: int = 400, feat: bool = True, lr: float = 1e-3):
    """
    Trains either
    ...
    if feat:
        ...
        optimizer = optim.Adam(lin_model.parameters(), lr=lr, weight_decay=1e-4)
...
class UCECollisionEntropyLoss_EWFreq(nn.Module):
    """
    Collision-entropy calibration loss:
      • Bins: equal-width across [H2_min, H2_max] (default [0, log2(C)]).
      • Weight: by bin frequency (|B_m| / N), as in standard ECE.
      • Target: analytic error f(H2) derived from binary collision entropy.

    Args
    ----
    n_bins : int
        Number of equal-width bins over the chosen H2 range.
    h2_min : float or None
        Lower edge of entropy range. None → 0.0 (theoretical minimum).
    h2_max : float or None
        Upper edge. None → log2(C) (theoretical maximum for uniform distribution).
        NOTE: using theoretical bounds keeps bin edges *fixed* across batches.
              If you prefer adaptive data-driven bounds, pass h2_min=None,
              h2_max=None and set `use_data_range=True` in forward().
    """

    def __init__(self, n_bins: int = 10, h2_min: float | None = None, h2_max: float | None = None):
        super().__init__()
        self.n_bins  = n_bins
        self.h2_min_ = h2_min  # store user prefs; may be None
        self.h2_max_ = h2_max

    # ---- analytic inversion: error from collision entropy (bits) ----
    @staticmethod
    def _risk_from_H2(u: torch.Tensor) -> torch.Tensor:
        # u in bits (log2); formula: r = 0.5*(1 - sqrt(max(0, 2*2^{-u} - 1)))
        inner = torch.clamp(2.0 * 2.0**(-u) - 1.0, min=0.0)
        return 0.5 * (1.0 - torch.sqrt(inner))
    ...
    def forward(
        ...
        *,
        ...
        Parameters
        ----------
        logits : (B, M, C) or (B, C) tensor
            Raw (unscaled) or temperature-scaled logits.
        labels : (B,) tensor (long)
            Ground-truth labels in {0,1,...,C-1}.
        use_data_range : bool (default False)
            If True, override h2_min_/h2_max_ and derive bin edges from
            the *current batch* H2 min/max.  This makes bins adaptive
            but batch-dependent.

        Returns
        -------
        uce : scalar tensor
            Frequency-weighted mean absolute entropy-risk gap.
        err_bins : (n_bins,) tensor
            Empirical error per bin (0 for empty bins).
        H2_bins : (n_bins,) tensor
            Mean H2 per bin (0 for empty bins).
        counts  : (n_bins,) tensor
            Sample count per bin (useful diagnostics).
        """
        ...
        # ----- 1) ensemble probs -----
        if logits.dim() == 3:
            # (B,M,C) → average member probabilities to get ensemble
            probs = F.softmax(logits, dim=-1).mean(dim=1)
        else:
        ...
        # ----- 2) per-sample collision entropy in bits -----
        # H2 = -log2(sum p^2)
        H2 = -torch.log2((probs**2).sum(dim=-1) + 1e-12)  # (B,)

        # ----- 3) 0/1 error relative to argmax prediction -----
        pred = probs.argmax(dim=-1)
        err  = (pred != labels).float()  # (B,)

        # ----- 4) choose bin range -----
        if use_data_range:
            h2_min = H2.min().detach()
            h2_max = H2.max().detach()
        else:
            # theoretical bounds unless explicitly overridden at init
            h2_min = torch.as_tensor(0.0 if self.h2_min_ is None else self.h2_min_, device=device)
            if self.h2_max_ is None:
                # uniform binary has H2 = 1 bit; for C>2 general upper bound is log2(C)
                # but we are in binary in your use case; still compute general formula
                h2_max = torch.as_tensor(float(torch.log2(torch.tensor(C, dtype=H2.dtype, device=device))), device=device)
            else:
                h2_max = torch.as_tensor(self.h2_max_, device=device)

        # protect against degenerate ranges
        if (h2_max - h2_min) < 1e-8:
            h2_max = h2_min + 1e-6  # tiny span to avoid nan bins

        # ----- 5) equal-width bin edges (fixed across batches if use_data_range=False) -----
        edges = torch.linspace(h2_min, h2_max, self.n_bins + 1, device=device)
        # Nudge upper edge so inclusive comparisons don't miss max due to fp
        edges[-1] = edges[-1] + 1e-6

        # ----- 6) accumulate frequency-weighted gaps -----
        total = float(B)
        gaps   = []
        err_bs = []
        H2_bs  = []
        ...
        for lo, hi in zip(edges[:-1], edges[1:]):
            ...
            if count.item() > 0:
                prop   = count.float() / total
                H2_bar = H2[mask].mean()
                err_bar= err[mask].mean()
                err_ref= self._risk_from_H2(H2_bar)

                gaps.append(torch.abs(err_bar - err_ref) * prop)
                err_bs.append(err_bar)
                H2_bs.append(H2_bar)
            else:
                # zero contribution; still append placeholders so we return fixed size
                gaps.append(torch.zeros((), device=device))
                err_bs.append(torch.zeros((), device=device))
                H2_bs.append(torch.zeros((), device=device))

        # ----- 7) frequency-weighted sum (ECE style) -----
        uce = torch.stack(gaps).sum()  # already multiplied by prop

        err_bins = torch.stack(err_bs)
        H2_bins  = torch.stack(H2_bs)
        counts   = torch.stack(cnt_bs).float()

        return uce, err_bins, H2_bins, counts
