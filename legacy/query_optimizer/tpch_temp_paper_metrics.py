#!/usr/bin/env python3
# tpch_calibration_metrics_uce_h2_csv.py
# -----------------------------------------------------------
# TPCH metrics (no plots):
...
# ------------------------- Defaults -------------------------
DATA_DIR   = "./data"
RAW_MODEL  = "./saved_models/sense_model_new_temp.pkl"
BEST_DIR   = "./ts_eval_tpch/best_models"
OUT_DIR    = "./ts_eval_tpch/metrics"

T_MIN, T_MAX = 0.5, 7.0  # TS bounds
...
# ----------------------------- Data I/O -----------------------------
def load_split(tag):
    """Returns (X_np, y_torchLong) for TPCH."""
    if tag == "calib_over":
        x = np.load(f"{DATA_DIR}/calib_over_x.npy")
        y = np.load(f"{DATA_DIR}/calib_over_y.npy")
    elif tag == "calib_un":
        x = np.load(f"{DATA_DIR}/calib_un_x.npy")
        y = np.load(f"{DATA_DIR}/calib_un_y.npy")
    elif tag == "train":
        x = np.load(f"{DATA_DIR}/sense_tpch_train_x.npy")
        y = np.load(f"{DATA_DIR}/sense_tpch_train_y.npy")
    elif tag == "val":
        x = np.load(f"{DATA_DIR}/sense_tpch_val_x.npy")
        y = np.load(f"{DATA_DIR}/sense_tpch_val_y.npy")
    else:
        raise ValueError(f"Unknown split tag: {tag}")
...
def _as_probabilities(L: torch.Tensor) -> torch.Tensor:
    ...
    L = to_tensor(L)
    if L.dim() not in (2, 3):
        raise ValueError(f"Expected dim 2/3, got {L.dim()} with shape {tuple(L.shape)}")
    sums = L.sum(-1)
...
def softmax_probs(logits):
    """Average member probabilities when we have (N,K,C); else just (N,C)."""
    L = to_tensor(logits)
...
def nll_of_ensemble(L_scaled, y):
    """Proper NLL for ensemble: avg member probs → NLL (keeps grad)."""
...
def learn_Tg(logits_cal, y_cal):
    """Global temperature (probability TS)."""
...
def learn_Tv(log3d_cal, y_cal):
    """Per-member temperatures (probability TS) for (N,K,C) logits."""
...
# ------------------- Main ----------------------
def main():
    ap = argparse.ArgumentParser(description="TPCH calibration metrics (ECE, UCE_H2, EU separability)")
    ...
    best_dir = pathlib.Path(args.best_dir).resolve()

    # Load base model & splits
    with open(args.raw_model, "rb") as f:
        sense = pickle.load(f)

    X_iid,   y_iid   = load_split("train")       # IID
    X_drift, y_drift = load_split("val")         # DRIFT
    X_cal,   y_cal   = load_split("calib_un")  # calibration

    raw_cal  = sense.gen_logits(X_cal)

    # Probability TS on raw model (learn on calib)
    Tg = learn_Tg(raw_cal, y_cal)
    Tv = learn_Tv(raw_cal, y_cal)  # may be None

    # ---------- Load uncertainty-calibrated models (global/ensemble/FBTS) ----------
    def _maybe_load(path):
        return pickle.load(open(path, "rb")) if path.exists() else None

    path_unc_global   = best_dir / "sense_model_global.pkl"
    path_unc_ensemble = best_dir / "sense_model_ensemble.pkl"
    if not path_unc_ensemble.exists():
        fallback = best_dir / "sense_model_ens.pkl"
        if fallback.exists():
            path_unc_ensemble = fallback
    path_fbts = best_dir / "sense_model_fbts.pkl"

    sense_unc_global   = _maybe_load(path_unc_global)
    sense_unc_ensemble = _maybe_load(path_unc_ensemble)
    sense_fbts         = _maybe_load(path_fbts)

    # Build logits per split & technique (human-readable keys like CIFAR/Census)
    def build_logits(split):
        X = X_iid if split == "IID" else X_drift
        base = sense.gen_logits(X)
        d = {
            "No TS": base,
            "Prob-TS (global)": apply_prob_T(base, Tg=Tg) if base is not None else None,
        }
        if Tv is not None and base is not None:
            d["Prob-TS (per-member)"] = apply_prob_T(base, Tv=Tv)
        if sense_unc_global is not None:
            d["Unc-Cal (global)"] = sense_unc_global.inf(X, apply_temp=True, mode="global")
        if sense_unc_ensemble is not None:
            d["Unc-Cal (ensemble)"] = sense_unc_ensemble.inf(X, apply_temp=True, mode="ensemble")
        if sense_fbts is not None:
            d["FBTS (feature-TS)"]  = sense_fbts.inf(X, apply_temp=True, mode="feat")
        # Detach any tensors (if methods returned torch tensors)
        for k,v in list(d.items()):
            if v is None: continue
            d[k] = v.detach() if isinstance(v, torch.Tensor) else v
        return d
    ...
    def add_rows_for_split(schemes, y, split_name):
        for name, logits in schemes.items():
            ...
            (eu_gap_mean, eu_gap_median,
             eu_mean_corr, eu_mean_inc,
             eu_med_corr,  eu_med_inc) = eu_separability(logits, y)
    ...
    # ------------------- Write CSV -----------------------
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = pathlib.Path(out_dir) / "tpch_calibration_metrics_uce_h2.csv"
    fieldnames = [
